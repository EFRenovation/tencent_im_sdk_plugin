...
class GroupManager {
	...
	func createGroup(call: FlutterMethodCall, result: @escaping FlutterResult) {
		...
		if memberListMap != nil {
			for i in memberListMap! {
				...
				item.role = V2TIMGroupMemberRole(rawValue: 200)!;
	...
	func getGroupMemberList(call: FlutterMethodCall, result: @escaping FlutterResult) {
		...
		let nextSeqParam = CommonUtils.getParam(call: call, result: result, param: "nextSeq") as? UInt64;
		...
		V2TIMManager.sharedInstance()?.getGroupMemberList(groupID, filter: filter!, nextSeq: nextSeq, succ: {
			...
			CommonUtils.resultSuccess(call: call, result: result, data: ["nextSeq": nextSeq, "memberInfoList": res]);
		}, fail: TencentImUtils.returnErrorClosures(call: call, result: result));
	}
	...
	func setGroupMemberRole(call: FlutterMethodCall, result: @escaping FlutterResult) {
		...
		V2TIMManager.sharedInstance()?.setGroupMemberRole(groupID, member: userID, newRole: V2TIMGroupMemberRole(rawValue: role)!, succ: {
			() -> Void in
