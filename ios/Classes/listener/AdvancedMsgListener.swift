...
class AdvancedMsgListener: NSObject, V2TIMAdvancedMsgListener {
	...
	public func onRecvNewMessage(_ msg: V2TIMMessage!) {
        asyncHydra({
			_ -> Int in
			TencentImSDKPlugin.invokeListener(type: ListenerType.onRecvNewMessage, method: "advancedMsgListener", data: try Hydra.await(V2MessageEntity.init(message: msg!).getDictAll()))
